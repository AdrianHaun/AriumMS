function [SplitFeatures,FilteredSpectra] = SplitFeaturesBySpectra(FeatureStruct,Spectra,Scores)
...
if isempty(Scores) % no matching spectra; split into individual features
...
Masses = Spectra(:,1);
Spectra(:,1) = [];
FilteredSpectra = cell(1,numel(uniqueCluster));
for n=1:numel(uniqueCluster)
    [~,id] = max(sum(Spectra(:,uniqueCluster{n})));
    FilteredSpectra{n} = Spectra(:,id);
end

FilteredSpectra = horzcat(FilteredSpectra{:});
id = all(FilteredSpectra == 0,2);
FilteredSpectra = [Masses(~id), FilteredSpectra(~id,:)];
